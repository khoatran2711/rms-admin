  ...
  template: `
    ...
    >
      ...
      <div class="p-[25px] pt-0 pb-[42px]">
        ...
        >
          <div
            class="overflow-auto w-full scrollbar max-h-[210px] min-h-[210px]"
            dir="ltr"
          >
            <div
              *ngIf="isTabDataEmpty(tab.key)"
              class="text-center text-light dark:text-white/60 text-[18px] font-medium min-h-[210px] flex items-center justify-center gap-[6px]"
            >
              Sorry !! No {{ tab.label }} Task Found
              <span
                nz-icon
                [nzType]="'meh'"
                [nzTheme]="'twotone'"
                [nzTwotoneColor]="'#fe8f31'"
              ></span>
            </div>
            <nz-table
              *ngIf="!isTabDataEmpty(tab.key)"
              [nzData]="taskList?.[tab.key]"
              [nzFrontPagination]="false"
              [nzShowPagination]="false"
            >
              <tbody>
                <tr class="group" *ngFor="let task of taskList?.[tab.key];let i = index">
                  <td
                  ...
                  >
                    <ul class="p-0 m-0 flex items-center gap-[15px]">
                      <li>
                        <button
                          nz-button
                          class="flex items-center justify-center bg-transparent border-none shadow-none text-light hover:text-info dark:text-white/60 p-0 w-[30px] h-[30px]"
                          (click)="openEditModal(task)"
                        >
                          <span
                            class="text-current text-[15px]"
                            nz-icon
                            nzType="edit"
                            nzTheme="outline"
                          ></span>
                      ...
                      <li>
                        <button
                          nz-button
                          class="flex items-center justify-center bg-transparent border-none shadow-none text-light hover:text-warning dark:text-white/60 p-0 w-[25px] h-[25px]"
                          (click)="toggleStar(task)"
                        >
                          <span
                            class="text-current text-[15px] [&.star-active]:text-warning"
                            [class.star-active]="task.favorite"
                            nz-icon
                            nzType="star"
                            nzTheme="outline"
                          ></span>
                      ...
                      <li>
                        <button
                          nz-button
                          class="flex items-center justify-center bg-transparent border-none shadow-none text-light hover:text-danger dark:text-white/60 p-0 w-[25px] h-[25px]"
                          (click)="removeTask(i)"
                        >
                          <span
                            class="text-current text-[15px]"
...
export class TaskList2Component implements OnInit {
  ...
  saveCheckedStates() {
    ...
    localStorage.setItem(
      'taskCheckedStates',
      JSON.stringify(this.taskCheckedStates)
    );
  }
  ...
  toggleStar(task: {
    ...
    if (task.favorite) {
      ...
      this.taskList['favorite'] = this.taskList['favorite'].filter(
        (t) => t.id !== task.id
      );
    }
  ...
  isTabDataEmpty(tabKey: string): boolean {
    return (
      !this.taskList ||
      !this.taskList[tabKey] ||
      this.taskList[tabKey].length === 0
    );
  }
}
