...
@Component({
  ...
  template: `
    <div class="flex items-center justify-center lg:hidden">
      <a
        class=" dark:text-white/[.87] h-[40px] px-[10px] gap-1.5 text-sm font-semibold inline-flex items-center bg-white dark:bg-white/10 rounded-4"
        (click)="openNav()"
      >
        <span
          class="text-[18px]"
    ...
    >
      <div class="flex items-center justify-end mt-[20px] lg:hidden">
        <a
          class="text-danger text-sm font-semibold inline-flex items-center px-[15px]"
          (click)="openNav()"
        >
          <span
            class="text-[19px]"
      ...
      <div class="p-[15px]">
        <ul class="bg-transparent border-none">
          ...
          >
            <i
              class="[&>svg]:w-[14px] [&>svg]:h-[14px] text-light-extra dark:text-white/60 group-hover:text-primary"
              nz-icon
              nzType="inbox"
              theme="outline"
            ></i>
            <span
              class="flex items-center justify-between flex-auto m-0 text-[15px] font-normal group-hover:text-primary"
            >
              <span>Inbox</span>
              <span
                class="flex items-center justify-center bg-primary/[.15] text-primary h-[20px] px-[6.5px] text-[11px] rounded-[10px]"
                >3</span
              >
            </span>
          </li>
          ...
          <li class="mt-[25px]" nz-menu-group>
            <h6
              title
              class="text-[#9299b8] dark:text-white/60 text-xs ms-[-5px] text-start"
            >
              Labels
            </h6>
            <ul>
              ...
              >
                <a
                  href="#"
                  class="flex text-theme-gray dark:text-white/60 items-center gap-[15px] font-normal text-[15px]"
                >
                  <span
                    class="[&>svg]:w-[14px] [&>svg]:h-[14px]"
                    nz-icon
              ...
              >
                <a
                  href="#"
                  class="flex items-center gap-[15px] font-normal text-[15px]"
                >
                  <span
                    class="[&>svg]:w-[14px] [&>svg]:h-[14px]"
                    nz-icon
              ...
              >
                <a
                  class="flex items-center gap-[10px] text-light dark:text-white/60 font-normal text-[15px]
                ...
                <div
                  class="absolute w-full top-0 start-1/2 -translate-x-1/2 bg-white dark:bg-[#1b1b28] w-[calc(100% + 60px)] px-[30px] py-[25px] rounded-lg shadow-boxLarge dark:shadow-[0_5px_30px_rgba(1,4,19,.60)] invisible opacity-0 [&.show]:visible [&.show]:opacity-100"
                  [ngClass]="{ show: isDropdownOpen }"
                >
                  <form>
                    <h1
                      class="mb-4 text-base font-medium text-dark dark:text-white/[.87]"
                    >
                      Add New Label
                    </h1>
                    <input
                    ...
                    />
                    <div
                      class="flex items-center flex-wrap mt-[10px] -mx-[5px] -mb-[5px]"
                    >
                      <button
                        nz-button
    ...
    <div
      [ngClass]="
        isClassA
          ? 'fixed bg-white dark:bg-[#212e3d] w-full rounded-[10px] shadow-[0_10px_50px_rgba(146,153,184,.19)] dark:shadow-[0_5px_30px_rgba(1,4,19,.60)] md:max-w-[600px] max-md:max-w-[400px] max-ssm:max-w-[240px] end-[15px] z-[9998] bottom-[5%]'
          : 'fixed bg-white dark:bg-[#212e3d] w-full rounded-[10px] shadow-[0_10px_50px_rgba(146,153,184,.19)] dark:shadow-[0_5px_30px_rgba(1,4,19,.60)] max-w-[1200px] end-[0] z-[9998] bottom-[5%] '
      "
      *ngIf="isCompose"
    >
      <div
      ...
      >
        ...
        <div class="flex items-center gap-[10px]">
          <div
            [ngClass]="{
              '[&.active>.hidden]:flex [&.active>.flex]:hidden': isClassA,
              'active [&.active>.hidden]:flex [&.active>.flex]:hidden':
                !isClassA
            }"
          >
            <button
              nz-button
              class="text-white bg-transparent border-none dark:text-white/[.87] screen flex items-center"
              nz-icon
              [nzType]="fullscreen"
              nzTheme="outline"
              (click)="toggleClass()"
            ...
            <button
              nz-button
              class="text-white bg-transparent border-none dark:text-white/[.87] screen hidden items-center"
              nz-icon
              [nzType]="fullscreen_exit"
              nzTheme="outline"
              (click)="toggleClass()"
            ></button>
          </div>
          <button
            nz-button
            class="text-white bg-transparent border-none dark:text-white/[.87] flex items-center"
            (click)="closeMail()"
            nz-icon
            [nzType]="close"
            nzTheme="outline"
      ...
      <div
        class="pb-[30px] px-[30px] max-3xl:h-[300px] max-md:h-[245px] overflow-x-auto"
      >
        <div class="flex gap-5">
          <div class="static w-full rounded-10 z-998">
            ...
            <div
              class="relative flex items-center justify-between w-full border-b border-regular dark:border-white/10 "
            >
              ...
              <span
                class="absolute ltr:right-[30px] rtl:left-[30px] top-1/2 -translate-y-1/2 text-light dark:text-white/[.87]"
                >Cc</span
              >
            </div>
            <div>
      ...
      >
        <div class="flex items-center gap-[15px]">
          <button
            nz-button
            class="h-[44px] px-[20px] rounded-4 bg-primary text-white border-primary"
          >
            <span>Send</span>
          </button>
          <a
            href="#"
            class="text-[13px] text-theme-gray dark:text-white/60 hover:text-primary"
          >
...
export class InboxSidebar implements OnInit {
  ...
  fullscreen_exit: any = 'fullscreen-exit';
  close: any = 'close';

  constructor(
    private mailSvc: AppsService,
