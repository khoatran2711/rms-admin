...
@Component({
  ...
  template: `
    <div class="bg-white dark:bg-white/10 rounded-[10px]">
      <ul
        class="flex items-center gap-x-[30px] px-[25px] py-0 border-b border-regular dark:border-white/10"
      >
        <li
          class="relative py-5 text-sm font-medium after:absolute ltr:after:left-0 rtl:after:right-0 after:bottom-0 after:w-full after:h-[1px] text-body dark:text-white/60 [&.active]:text-primary [&.active]:after:bg-primary cursor-pointer duration-300 ease-in-out"
      ...
      <div class="w-full pt-[10px] p-[25px]">
        <app-tab class="w-full" [isActive]="activeTab === 1">
          ...
          >
            <tbody cdkDropList (cdkDropListDropped)="drop($event)">
              <tr class="group" *ngFor="let item of UserList" cdkDrag>
                <td
                  class="border-none py-[6px] ltr:pl-0 rtl:pr-0 group-hover:bg-transparent w-full text-start"
                >
                  <label
                    class="text-[14px] font-normal leading-[17px] text-light dark:text-white/60 capitalize"
                    nz-checkbox
                    nzValue="{{ item.id }}"
                    >{{ item.sub }}</label
                  >
                ...
                <td
                  class="border-none py-[6px] ltr:pr-0 rtl:pl-0 group-hover:bg-transparent w-full"
                >
                  <ul class="flex items-center justify-end">
                    <li>
                      <div
                        class="whitespace-nowrap block mx-[10px] text-xs text-light-extra dark:text-white/60 leading-none capitalize"
                      >
                        {{ item.duration }}
                      </div>
                    </li>
                    <li>
                      <div
                        class="block mx-[10px] leading-none w-[30px] h-[30px] rounded-full"
                      >
                        <img
                    ...
                    <li>
                      <div
                        class="flex items-center mx-[10px] leading-none text-light-extra dark:text-white/60"
                      >
                        <span
                          class="[&>svg]:w-[18px] [&>svg]:h-[18px]"
                          nz-icon
                          nzType="drag"
                          nzTheme="outline"
                        ></span>
          ...
          <div class="mt-[12px] mb-[22px]">
            <button
              nz-button
              class="inline-flex items-center bg-primary/10 text-primary w-full h-[50px] text-xs font-semibold border-none rounded-[6px] px-[25px]"
            >
              ...
              >
                <div nz-row nzGutter="15">
                  <div nz-col nzXXl="3" nzLg="5" nzMd="4" nzSm="5" nzXs="24">
                    <div
                      class="flex items-center justify-center flex-col bg-regularBG dark:bg-white/10 h-full border dark:border-white/10 rounded-[10px] min-h-[50px] p-[15px]"
                    >
                      <h1
                        class="mb-0 text-lg font-semibold uppercase text-dark dark:text-white/60"
                      >
                        {{ group.day }}
                      </h1>
                      <span class="capitalize text-body dark:text-white/60 ">{{
                        group.month
                      }}</span>
                    </div>
                  </div>
                  ...
                  >
                    <div
                      class="flex mb-[25px] last:mb-0 w-full"
                      *ngFor="let activity of group.activities"
                    >
                      <div class="flex items-center min-w-[76px] gap-x-[10px]">
                        <div
                          class="flex items-center justify-center w-6 h-6 rounded-full bg-{{
                            activity.status
                          }}/10 text-{{ activity.status }}"
                        >
                          <span
                            class="text-[12px]"
                            nz-icon
                            nzType="{{ activity.icon }}"
                            nzTheme="outline"
                          ></span>
                        </div>
                        <img
