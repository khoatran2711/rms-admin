...
@Component({
  ...
  template: `
    <div nz-row [nzGutter]="25">
      <div
        *ngFor="let item of projectList | slice : 0 : 3"
        nz-col
        nzXs="24"
        nzXXl="8"
        class="mb-[25px]"
          ...
          <div class="pt-[30px] px-[30px]">
            <div class="flex items-start justify-between">
              <h1 class="flex flex-wrap items-center -m-0.5 text-base">
                <a
                  [routerLink]="['/apps/projects/project-details']"
                  class="m-0.5 me-[11px] text-dark dark:text-white/[87] text-15 font-medium capitalize"
                >
                  {{ item.project }}
                </a>
                <span class="uppercase text-[10px]">
                  <nz-tag
                    class="mb-0 rounded-[3px] font-bold text-white text-[10px] bg-warning border-warning"
                    *ngIf="item.status == 'in progress'"
                    >{{ item.status }}</nz-tag
                  >
                  <nz-tag
                    class="mb-0 rounded-[3px] font-bold text-white text-[10px] bg-success border-success"
                    *ngIf="item.status == 'completed'"
                    >{{ item.status }}</nz-tag
                  >
                  <nz-tag
                    class="mb-0 rounded-[3px] font-bold text-white text-[10px] bg-primary border-primary"
                    *ngIf="item.status == 'early'"
                    >{{ item.status }}</nz-tag
                  >
                  <nz-tag
                    class="mb-0 rounded-[3px] font-bold text-white text-[10px] bg-danger border-danger"
                    *ngIf="item.status == 'late'"
                    >{{ item.status }}</nz-tag
                  >
                </span>
              </h1>
            ...
            <p class="text-body dark:text-white/60 mt-[15px] mb-[25px]">
              Adipisicing eu magna velit est exercitation et consequat Lorem
              laboris
            </p>
            <div class="flex items-center mb-[15px] gap-x-[30px]">
              <div class="flex flex-col">
                <span class="text-xs mb-0.5 text-light-extra dark:text-white/60"
                  >Start Date</span
                ><strong class="font-medium text-body dark:text-white/60">{{
                  item.startDate
                }}</strong>
              </div>
              <div class="flex flex-col">
            ...
            <p class="mt-0.5 text-light-extra dark:text-white/60 text-xs">
              12/{{ item.tasks }} Task Completed
            </p>
          </div>
          <div
            class="pt-4 px-[30px] pb-[25px] mt-[17px] border-t border-regular dark:border-white/10"
          >
            <p class="mb-4 text-sm text-light-extra dark:text-white/60">
              Assigned To
            </p>
            <div class="flex flex-wrap items-center gap-[6px] capitalize">
              ...
              <a *ngIf="item.member.length > 2">
                <nz-avatar
                  ...
                  [nzText]="'+' + [item.member.length - 2].toString()"
                  nz-tooltip
                  [nzTooltipTitle]="
                    [item.member.length - 2].toString() + ' More'
                  "
                ></nz-avatar>
              </a>
            </div>
